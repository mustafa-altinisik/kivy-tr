...
class illerIlcelerMahaller(App):
    ...
    def semtSecim(self, nesne):
        if nesne.selection:
            secim=nesne.selection[0].index
            semtId=nesne.data[secim]['semtId']
            mahalleler=xmlKok.findall('tbl_mahalle')
        
            self.root.ids.mahalleler.adapter.data= [
                        {'mahalleId': mahalle.find('mahalle_id').text, 'adi': mahalle.find('mahalle_ad').text.encode("utf-8")} 
                        for mahalle in mahalleler if mahalle.find('semt_id').text==semtId
                    ]

            self.root.ids.mahalleler._trigger_reset_populate()
    ...
    def build(self):
        ...
        self.root.ids.semtler.adapter.bind(on_selection_change=self.semtSecim)
    
      
illerIlcelerMahaller().run()
